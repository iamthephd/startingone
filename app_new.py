...
def init_session_state():
    ...
    if 'chat_popover_state' not in st.session_state:
        st.session_state.chat_popover_state = False
...
def process_chat_input():
    ...
    if st.session_state.chatbot_input and not st.session_state.processing_query:
        ...
        # Store the state of the chat popover
        if 'chat_popover_state' not in st.session_state:
            st.session_state.chat_popover_state = True
    ...
    if st.session_state.processing_query:
        with st.spinner("Getting response..."):
            ...
            if last_user_message:
                ...
                try:
                    response = process_chatbot_query(st.session_state.engine, query, st.session_state.file_config['table_name'])
...
def main():
    ...
    if st.session_state.selected_file:
        ...
        # Check if we're processing a chat, if so, set the chat_popover_state to True
        if st.session_state.processing_query:
            st.session_state.chat_popover_state = True
        
        # Use the chat_popover_state to determine if the popover should be auto-opened
        with st.popover("Chat 🤖", use_container_width=False, expanded=st.session_state.chat_popover_state):
            render_chatbot()
            # After rendering, if it was opened due to a chat query, keep it open
            if st.session_state.processing_query:
                st.session_state.chat_popover_state = True
